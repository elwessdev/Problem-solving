...
#Q.2 => Select only the Names of all the Bands
select name as 'Band Name' from bands;

#Q.3 => Select the Oldest Album
SELECT * from albums
where albums.release_year IS NOT NULL
ORDER BY albums.release_year
LIMIT 1;

#Q.4 => Get all Bands that have Albums
select DISTINCT b.name as 'Band Name' from Bands as b left join Albums as a on b.id=a.band_id;

#Q.5 => Get all Bands that have No Albums
select b.name as 'Band name' from Bands as b 
left join Albums as a on b.id=a.band_id
group by a.band_id having count(a.id)=0;

SELECT b.name as 'Band name' FROM Bands as b
LEFT JOIN Albums as a ON b.id = a.band_id
where a.band_id is null;

#Q.6 => Get the Longest Album
SELECT a.name AS 'Name', a.release_year AS 'Release Year', SUM(s.length) AS 'Duration' FROM albums AS a
JOIN songs AS s ON a.id=s.album_id
GROUP BY a.id
ORDER BY Duration DESC
LIMIT 1;

#Q.7 => Update the Release Year of the Album with no Release Year
UPDATE albums SET release_year=1986, id=id where release_year IS NULL AND id=4;
select * from albums;

#Q.8 => Insert a record for your favorite Band and one of their Albums
select * from bands;
select * from albums;
insert into bands(name) values ("a1");
insert into albums(name,release_year,band_id) VALUES ("maa",2987,8);

#Q.9 => Delete the Band and Album you added in #8
DELETE FROM albums where name="maa" AND id=20;
DELETE FROM bands where name="a1" and id=8;

#Q.10 => Get the Average Length of all Songs
SELECT AVG(length) as "Average Song Duration" from songs;

#Q.11 => Select the longest Song off each Album
select a.name as 'Album', a.release_year as 'Release Year', MAX(s.length) as "Duration" from albums as a join songs as s
on a.id=s.album_id
group by a.id
order by a.release_year DESC;

#Q.12 => Get the number of Songs for each Band
select b.name as 'Band', count(s.id) as 'Number of Songs' from bands as b
join albums as a 
join songs as s
on b.id=a.band_id AND a.id=s.album_id
group by a.band_id;